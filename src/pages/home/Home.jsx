  ...
  return (
    <Box sx={{ width: '100%', overflowX: 'hidden' }}>
      ...
      <Container maxWidth="xl" sx={{ mt: -4, position: 'relative', zIndex: 3, mb: 10 }}>
        ...
        >
          {[
            ...
            <Box key={idx} sx={{ display: 'flex', alignItems: 'flex-start', gap: 2.5, flex: 1 }}>
              <Box
                sx={{
                  p: 1.5,
                  borderRadius: 2.5,
      ...
      <Container maxWidth="xl" sx={{ mb: 12 }}>
        ...
        >
          {BRAND_STATS.map((stat, idx) => (
            <Box key={idx} sx={{ textAlign: 'center', minWidth: { xs: '140px', sm: '200px' } }}>
      ...
      <Container maxWidth="xl" sx={{ mb: 12 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
          {FEATURED_PRODUCTS.slice(0, 2).map((prod) => (
            <Paper
              key={prod.id}
              elevation={0}
              sx={{
